...
struct imagePicker: UIViewControllerRepresentable {
    ...
    class Coordinator: NSObject,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
        ...
        func uploadImageToFireBase(image: UIImage) {
            ...
            // Upload the file to the path IMAGE_FILE_NAME
            storage.child(IMAGE_FILE_NAME).putData(image.jpegData(compressionQuality: 0.42)!, metadata: metadata) { (metadata, error) in
        ...
        func downloadImageFromFirebase() {
            ...
            storage.child(IMAGE_FILE_NAME).downloadURL { (url, error) in
                if error != nil {
                    // Handle any errors
                    print((error?.localizedDescription)!)
                    return
